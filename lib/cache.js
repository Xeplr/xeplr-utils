 ...
 * @param {string} [config.keyPrefix] - Else REDIS_PREFIX, else 'xeplr:'. Give each app
 *   its own: apps sharing a Redis on the default read each other's keys, which
 *   is why @xeplr/auth requires REDIS_PREFIX and refuses 'xeplr:'.
...
async function delPattern(pattern) {
  try {
    const client = getClient();
    // The prefix the client ACTUALLY writes under. _config.keyPrefix alone is
    // empty when the prefix came from REDIS_PREFIX or the default, and SCAN —
    // which ioredis does not prefix — then matched nothing, silently: every
    // "clear the cache" call was a no-op.
