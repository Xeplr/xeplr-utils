...
// The nodemailer transport options for an SMTP config — one place, so the
// startup check (checkEmail) connects exactly the way a real send does.
...
async function sendViaSMTP(to, subject, html, cc, attachments) {
  ...
  var transporter = loadNodemailer().createTransport(smtpTransportOptions(cfg));
...
/**
 * CAN THIS PROCESS SEND EMAIL? For a startup summary — never throws, never
 * sends a message.
 *
 *   smtp    connects and authenticates (nodemailer verify), within timeoutMs
 *   brevo   asks Brevo whether the API key is valid
 *   aws / azure   checks their settings are present (not contacted)
 *
 * @param {object} [options]
 * @param {number} [options.timeoutMs=5000]
 * @returns {Promise<{ ok: boolean, provider: string|null, detail: string }>}
 */
async function checkEmail(options) {
  var timeoutMs = (options && options.timeoutMs) || 5000;
  var cfg = _config || emailConfigFromEnv();
  if (!cfg || !cfg.provider) {
    return { ok: false, provider: null, detail: 'not configured (EMAIL_PROVIDER is not set)' };
  }
  var provider = String(cfg.provider).toLowerCase();
  var env = getConfigFromEnvFallback();

  try {
    if (provider === 'smtp') {
      var smtp = cfg.smtp || env.smtp;
      if (!smtp.host) return { ok: false, provider: provider, detail: 'SMTP_HOST is not set' };
      if (!smtp.from) return { ok: false, provider: provider, detail: 'SMTP_FROM is not set' };
      var transporter = loadNodemailer().createTransport(smtpTransportOptions(smtp));
      try {
        await withTimeout(transporter.verify(), timeoutMs, 'no answer from ' + smtp.host + ' within ' + timeoutMs + 'ms');
      } finally {
        if (typeof transporter.close === 'function') transporter.close();
      }
      return { ok: true, provider: provider, detail: smtp.host + ':' + _num(smtp.port, 587) + ', connected' };
    }

    if (provider === 'brevo') {
      var brevo = cfg.brevo || env.brevo;
      if (!brevo.apiKey) return { ok: false, provider: provider, detail: 'BREVO_API_KEY is not set' };
      if (!brevo.fromEmail) return { ok: false, provider: provider, detail: 'BREVO_FROM_EMAIL is not set' };
      var res = await fetch('https://api.brevo.com/v3/account', {
        headers: { accept: 'application/json', 'api-key': brevo.apiKey },
        signal: AbortSignal.timeout(timeoutMs)
      });
      if (!res.ok) return { ok: false, provider: provider, detail: 'Brevo refused the API key (' + res.status + ')' };
      return { ok: true, provider: provider, detail: 'API key accepted' };
    }

    if (provider === 'aws') {
      var aws = cfg.aws || env.aws;
      var missingAws = ['region', 'accessKeyId', 'secretAccessKey', 'from'].filter(function(k) { return !aws[k]; });
      if (missingAws.length) return { ok: false, provider: provider, detail: 'missing ' + missingAws.join(', ') };
      return { ok: true, provider: provider, detail: 'settings present, not contacted' };
    }

    if (provider === 'azure') {
      var azure = cfg.azure || env.azure;
      if (!azure.connectionString || !azure.from) return { ok: false, provider: provider, detail: 'missing connection string or from address' };
      return { ok: true, provider: provider, detail: 'settings present, not contacted' };
    }

    return { ok: false, provider: provider, detail: 'unknown provider "' + provider + '" — smtp, brevo, aws or azure' };
  } catch (err) {
    var message = err && err.message ? err.message : String(err);
    // loadNodemailer's sentence is written for a stack trace; a summary line
    // needs the fact and the fix.
    if (/'nodemailer' is not installed/.test(message)) message = 'nodemailer is not installed (npm install nodemailer)';
    return { ok: false, provider: provider, detail: message };
  }
}

function getConfigFromEnvFallback() {
  var saved = _config;
  _config = null;
  try { return getConfig(); } finally { _config = saved; }
}
...
module.exports = { sendEmail, configureEmail, emailConfigFromEnv, smtpFromEnv, configureFromEnv, checkEmail, getDeadLetterQueue, replayDeadLetters };
